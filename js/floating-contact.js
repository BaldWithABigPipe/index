...
export const handleFloatingContact = () => {
  ...
  // Booking section visibility check for mobile screens
  const handleBookingSectionVisibility = () => {
    if (window.innerWidth > 600) return; // Only for screens 600px and below
    ...
    if (isBookingVisible) {
      contactButton.classList.add('floating-contact__button--hidden-in-booking');
    } else {
      contactButton.classList.remove('floating-contact__button--hidden-in-booking');
    }
  };
  
  // Initial check
  handleFooterVisibility();
  handleBookingSectionVisibility();
